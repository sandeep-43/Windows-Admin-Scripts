...
if(Test-Path $RootPath)
{
    ...
    $Header = "FolderPath,GroupName,AccessControlType,IsInherited,FileSystemRights,Owner"
    ...
    foreach ($Folder in $Folders) 
    {
	    ...
	    Foreach ($ACL in $ACLs )
        {
		    ...
		    $OutInfo =  $TempFolderName + "," + $ACL.IdentityReference  + "," + $ACL.AccessControlType + "," + $ACL.IsInherited + "," + $ACL.FileSystemRights | where-object {$ACL.IdentityReference -notlike "BUILTIN\Administrators" -and $ACL.IdentityReference -notlike "*AUTHORITY\SYSTEM*" -and $ACL.IdentityReference -notlike "CREATOR*OWNER"}
                ...
                #Comment Out Either of the next two lines (Select Any 1)	
	            #if ( -not ( $ACL.IsInherited )) { Add-Content -Value $OutInfo -Path $OutFile }
		        Add-Content -Value $OutInfo -Path $OutFile
            
                if($ShowProgress) #For Progress Bar
                {
                    For($i = 1; $i -le $TotalFolders; $i++)
                    { Write-Progress -Activity “Collecting folder permissions” -status “Finding ACL in folder #$i” -percentComplete ($i / $TotalFolders * 100)}
                }
	    }
...
Else
{
    Write-Warning "Path entered is incorrect. Please re-run the script with correct path !!!"
}
