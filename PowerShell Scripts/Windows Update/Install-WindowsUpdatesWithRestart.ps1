...

    ...
    $RebootStatus = New-Object -ComObject "Microsoft.Update.SystemInfo"
    $Date=Get-Date
    if($RebootStatus.RebootRequired)
    {
        Write-Output "[$Date] Server will reboot now" | Out-File $LogFile -Append
        Restart-Computer -Force
    }
    else 
    {
        Write-Output "[$Date] Server reboot is not required" | Out-File $LogFile -Append
    }
